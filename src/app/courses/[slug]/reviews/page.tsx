...
import {
  formatList,
  formatNumberReviews as formatNumber,
...
export async function generateStaticParams(): Promise<Array<Params>> {
  ...
  *[_type == 'course'] {
  ...
  return await sanityClient.fetch<Pick<Course, "slug">[]>(query);
}
...
interface ReviewsPageProps extends CourseWithReviews {
  rating: number;
  difficulty: number;
  workload: number;
}

async function getReviews(slug: Course["slug"]): Promise<ReviewsPageProps> {
  const query = `
  ...
  return { ...course, rating, difficulty, workload };
}
...
export async function generateMetadata({
  ...
  const query = `*[_type == 'course' && slug.current == $slug][0].name`;
  const name = await sanityClient.fetch<string>(query, { slug });
